...
let headers = null;
  ...
  /*
   * STEP 8 SERVICE BOUNDARY
   *
   * When Screen 1 supplies a Booking ID, Screen 2 gets its
   * booking snapshot exclusively through the Step 8 adapter.
   *
   * The fallback path is retained only for manual execution
  ...
  /*
   * Screen 1 normally supplies the exact Booking ID.
   *
   * In that normal path, DO NOT perform getEDBBookingRows().
   * The Step 8 adapter is the single read boundary.
   */

  if (wantedId) {

    const booking =
      getEDBBookingForNewUI(wantedId);

    if (!booking) {
      throw new Error(
        'Booking ID not found: ' + wantedId
      );
    }

    /*
     * The adapter returns the original selected-row snapshot
     * required by the existing Screen 2 renderer.
     */
    selectedRow = booking.values;
    headers = booking.headers;

  } else {

    /*
     * Manual/no-ID fallback.
     * This path is only used when the function is invoked
     * without a Booking ID.
     */
    ...
    const dataset =
      getEDBBookingRows(responseSheet);
