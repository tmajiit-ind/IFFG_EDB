...
function refreshEDBDashboard(spreadsheetOverride, selectedBookingId) {
  ...
  const bookingId =
    String(
      selectedBookingId ||
      (existingDashboard
        ? existingDashboard.getRange('K3').getValue()
        : '') ||
      ''
    ).trim();

  createEDBDashboard(spreadsheet);
  setupB7D1BookingControl(spreadsheet);
  setupB7D2BookingActions(spreadsheet);
...
function getB7SelectedBooking(
  ...
  /*
   * B7 FIX:
   * Resolve the response sheet from the spreadsheet object
   * rather than relying solely on bookingDataContext.responseSheet.
   *
   * The existing context reports 38 headers while the actual
   * Form Responses 1 sheet contains 39 columns.
   */
  ...
  if (
    ...
    throw new Error(
      'B7: Booking data context/spreadsheet is unavailable.'
    );
  }
  ...
  if (!sheet) {
    throw new Error(
      'B7: Form Responses 1 sheet not found.'
    );
  }
  ...
  /*
   * ALWAYS obtain headers directly from the live sheet.
   */
  const headers =
    sheet
      .getRange(
  ...
  if (
    ...
    throw new Error(
      'B7: Booking ID column not found in Form Responses 1.'
    );
  }

  /*
   * Read the complete live data width.
   */
  const values =
    sheet
      .getRange(
  ...
  /*
   * Exact trimmed comparison.
   * Case sensitivity is deliberately preserved.
   */
  for (
    let i = 0;
    i < values.length;
...
function refreshB7SelectionPanels(
  ...
  if (!bookingId) {
    ...
    dashboard
      .getRange(
        B7_D1_CONFIG.DETAIL_START_ROW,
        B7_D1_CONFIG.DETAIL_START_COL,
        B7_D1_CONFIG.DETAIL_ROW_COUNT,
        B7_D1_CONFIG.DETAIL_COL_COUNT
      )
      .clearContent();
...
function EDB_DIAGNOSTIC_SHOW_BOUND_SPREADSHEET() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  Logger.log("========================================");
  Logger.log("IFFG EDB - BOUND SPREADSHEET");
  Logger.log("========================================");
...
function getEDBBookingForNewUI(bookingId) {
  ...
  var selected = getB7SelectedBooking(context, id);
